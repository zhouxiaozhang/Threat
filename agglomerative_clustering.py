...
from feature_extract import Content_extract
...
class Agglomerativeclustering:
    ...
    def _merge_clusters(self,cluster_a,cluster_b,cluster_pair_list=None,clusters=None):
        ...
        if cluster_pair_list is not None:
            ...
            for i,pair in enumerate(cluster_pair_list):
                if cluster_b['id'] is pair['target']['id'] or cluster_a['id'] is pair['key']['id']:
                    ...
                    similarity=self._similarity(cluster_c,pair['key'])
                    if similarity>pair['similarity']:
                        cluster_pair_list[i]['similarity']=similarity
                        cluster_pair_list[i]['target']=cluster_c
    ...
    def _init_clusters(self,contents,docs,labels):
        ...
        for i,content in enumerate(contents):
            ...
            clusters.append({'id':i,'vector':content,'docs':[(content,docs[i],labels[i])]})
...
clusters=Agglomerativeclustering(0.92).fit(content,docs,labels)  
